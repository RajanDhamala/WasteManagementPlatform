    ...
    <div className="bg-white rounded-xl shadow-md overflow-hidden transform transition-all duration-300 ease-in-out hover:shadow-xl hover:scale-[1.02] hover:-translate-y-1 cursor-pointer">
      ...
      >
        ...
        {EventImg.length > 1 && (
          <>
            <button
              onClick={() => setCurrentImageIndex((prev) => (prev === 0 ? EventImg.length - 1 : prev - 1))}
              className="absolute left-2 top-1/2 -translate-y-1/2 p-1 rounded-full bg-black/20 hover:bg-black/40 text-white"
            >
              <ChevronLeft size={20} />
            </button>
            <button
              onClick={() => setCurrentImageIndex((prev) => (prev + 1) % EventImg.length)}
      ...
      <div className="p-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={copyToClipboard} className="px-4 py-2 border border-gray-200 rounded-lg hover:bg-gray-50 hover:border-green-500 hover:text-green-600">
