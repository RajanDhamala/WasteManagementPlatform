...
function TanStack() {
  ...
  const [formdata, setformdata] = useState({ username: '', heading: '', data: '' });
  ...
  const addpost = async (formdata) => {
    const response = await axios.post(
      'http://localhost:8000/community/Postdata',
      formdata,
  ...
  const { mutate, isLoading: koadhai, isError: errorHo, error: bug1 } = useMutation({
    mutationFn: addpost,
    onMutate: (newData) => {
      console.log('Optimistic mutation data:', newData);
    },
    onSuccess: (newData) => {
      queryClient.setQueryData(['listdata'], (oldData) => {
        return [newData, ...oldData]; 
      });
      setformdata({ username: '', heading: '', data: '' });
    },
  });
    ...
    <div className="mt-16">
      <div className="max-w-2xl mx-auto p-3 bg-white rounded-lg shadow-lg">
        <form className="space-y-6" onSubmit={(e) => submitdata(e)}>
          ...
          <div>
            <label htmlFor="heading" className="block text-lg font-semibold text-gray-700">
              Subject
            </label>
            <input
          ...
          <div>
            <label htmlFor="data" className="block text-lg font-semibold text-gray-700">
              Data
            </label>
            <textarea
              value={formdata.data}
      ...
      {!isError && listdata ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 px-4 mt-4">
          {listdata.map((item, index) => (
            <div className="bg-white p-6 rounded-lg shadow-lg transition transform hover:scale-105 hover:shadow-xl" key={index}>
              <div className="flex flex-col gap-4">
                ...
                <div className="text-sm text-gray-600 font-medium">{item.heading}</div>
